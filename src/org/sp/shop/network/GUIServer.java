...
public class GUIServer extends JFrame{
	...
	public GUIServer() {
		...
		bt.addActionListener((e)->{
			...
			bt.setEnabled(false);//버튼을 중복해서 못누르게처리
	...
	public void accessListen() {
		...
		try {
			...
			//무한대로 사용자의 접속을 허용.. 
			while(true) {
			Socket socket=server.accept();//접속자 기다리고, 접속자가 발생하면 대화용 소켓을 반환받는다.
			
			//접속자가 발견되면, 대화용 쓰레드를 생성하면서 socket등의 정보를 넘기자
			ServerMessageThread smt=new ServerMessageThread(this, socket);
			smt.start();//대화시작
			
			vec.add(smt);//접속자 명단에 추가
			
			area.append("현재접속자 "+vec.size()+"명\n");
			}
		} catch (IOException e) {
