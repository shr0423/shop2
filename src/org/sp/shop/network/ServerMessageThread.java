...
public class ServerMessageThread extends Thread{
	...
	public void listen() {
		...
		try {
			...
			//나의 메서드만 호출하지말고, 현재  접속한 모든 유저들이 보유한
			//sendMsg()도 함께 호출하자
	...
	//서버측의 쓰레드는끊임없이 클라이언트의 메시지를 청취하고
	//다시보내야 한다 따라서 무한루프로 처리..
	public void run() {
		while(flag) {
			listen();
		}
