...
public class ProductPage extends Page{
		...
		for(int i=0;i<subNavi.size();i++) {
			...
			subNavi.get(i).addMouseListener(new MouseAdapter() {
				...
				public void mouseClicked(MouseEvent e) {
					Object obj=e.getSource();//클릭한 녀셕을 얻기
					//클릭한 라벨의 index와동일한page를 보여준다.
	...
	//페이지 전환처리
		public void showHide(int n) {//보이게 하고 싶은 index만 넘겨받는다
			for(int i=0;i<pages.length;i++) {
			if(i==n) {//넘겨받은 매개변수와 i가 일치할때만 보이게
				pages[i].setVisible(true);//보이게처리
				
				//상품 목록페이지의 경우엔 화면 갱신 요구됨..
